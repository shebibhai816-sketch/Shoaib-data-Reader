...
def get_completed_candle(df):
    ...
    completed = df[
        df["Datetime"] + pd.Timedelta(minutes=5)
        <= completed_limit
    ].copy()
    ...
    if NY_START_UTC <= hour < NY_END_UTC:
        ...
        if trend == "BEAR":
            ...
            return {
                "signal": "SELL",
                "confidence": "HIGH",
                "reason": "H20 BEARISH NY CORE CONDITION SATISFIED"
            }

        return {
            "signal": "NO SIGNAL",
            "confidence": "NONE",
            "reason": "NY CORE ACTIVE BUT EMA TREND IS NOT BEARISH"
        }
...
if selected_pair != "EUR/USD" or selected_timeframe != "5-Minute":

    st.warning(
        "⚠️ H20 signal engine صرف EUR/USD 5-Minute کے لیے validated اور locked ہے۔"
    )

    st.info(
        "دوسرے pairs/timeframes کو signal دینے کے لیے الگ validation ضروری ہوگی۔"
    )
...
try:
    ...
    if completed is None:
        ...
        st.error("Completed 5-minute candle available نہیں ہے۔")
        ...
        with s2:
            st.write("### NY Core")
            st.write(
                "**ACTIVE**" if ny_active else "**INACTIVE**"
            )
        ...
        st.subheader("🕯️ Completed Candle")
        ...
        st.success("LIVE DATA FEED : PASS")
        st.success("INDICATOR ENGINE : PASS")
        st.success("H20 RULE : LOCKED")
        st.success("STRATEGY MODIFICATION : DISABLED")
        st.success("AUTO TRADING : DISABLED")

        st.caption(
            f"Analysis interval selected: {analysis_interval} seconds"
        )
